...
**执行：**
  ...
  2. 创建 `src/lib/queue/redis.ts` -- Redis 连接配置（支持本地和云端 Redis）
  3. 创建 `src/lib/queue/queues.ts` -- 定义 6 个 BullMQ 队列（pipeline、code-gen、ai-call、screenshot、deploy、quality-gate），配置 concurrency/timeout/attempts 参数（对应第12章 12.2.6）
  4. 创建 `src/lib/queue/index.ts` -- 统一导出
  5. 在 `.env` 添加 REDIS_URL
**验证：**
  - TypeScript 编译通过
  - 队列定义与第12章参数一致（如 code-gen concurrency:3, timeout:300000）
**产出：**
  - `src/lib/queue/` 下 3 个文件
  - `.env` 增加 Redis 配置
**预估耗时：** 1.5小时
...
**执行：**
  ...
  3. `src/lib/services/project-service.ts` -- 业务逻辑层
...
**执行：**
  ...
  4. `src/lib/services/dag-service.ts` -- DAG 业务逻辑
...
**产出：**
  ...
  - `src/lib/services/dag-service.ts`
**预估耗时：** 3小时
**可并行：** 否（依赖项目 API）
...
**前置条件：** STEP-006完成
**执行：**
  1. 创建 `src/lib/ai/adapter-interface.ts` -- 定义 AIAdapter 统一接口
     ```typescript
     interface AIAdapter {
     ...
     ```
  2. 创建 `src/lib/ai/types.ts` -- AIChatRequest、AIChatResponse、CostEstimate、ModelConfig
  3. 创建 `src/lib/ai/prompt-template.ts` -- Prompt 模板引擎（变量替换 + 上下文注入）
**验证：**
...
**执行：**
  ...
  2. 创建 `src/lib/ai/adapters/claude-adapter.ts`
     - 实现 AIAdapter 接口
...
**执行：**
  ...
  2. 创建 `src/lib/ai/adapters/deepseek-adapter.ts`
     - 实现 AIAdapter 接口
...
**验证：**
  ...
  - 流式输出正常
**产出：**
  - `src/lib/ai/adapters/deepseek-adapter.ts`
**预估耗时：** 1.5小时
...
**执行：**
  1. 创建 `src/lib/ai/router.ts` -- AI 模型智能路由
     - 根据任务类型选择模型（简单任务 -> 便宜模型，复杂任务 -> 强模型）
     - 根据平台配置选择默认 Provider
  2. 创建 `src/lib/ai/circuit-breaker.ts` -- 熔断器实现
     - 连续 3 次失败 -> 触发熔断（open）
     - 冷却 5 分钟后半开（half-open）
     - 成功一次 -> 关闭（closed）
     - 状态存储在 Redis
  3. 创建 `src/lib/ai/cost-tracker.ts` -- 成本追踪器
     - Redis INCRBYFLOAT 原子扣减
     - 超预算时拒绝调用
  4. 创建 `src/lib/ai/index.ts` -- AIProxy 统一入口
**验证：**
...
**执行：**
  1. 创建 `src/lib/engines/requirement-engine.ts` -- 需求对话引擎
...
**验证：**
  ...
  - "差不多了" -> 立即生成汇总
**产出：**
  - `src/lib/engines/requirement-engine.ts`
...
**执行：**
  1. 创建 `src/lib/engines/feature-map-engine.ts` -- 功能地图引擎
...
**验证：**
  ...
  - PM 修改指令在 15 秒内响应
**产出：**
  - `src/lib/engines/feature-map-engine.ts`
...
**执行：**
  1. 创建 `src/lib/engines/codegen/context-manager.ts` -- 上下文管理器
...
**验证：**
  ...
  - 总上下文 token 数在限制内
**产出：**
  - `src/lib/engines/codegen/context-manager.ts`
**预估耗时：** 3小时
**可并行：** 否
...
**执行：**
  1. 创建 `src/lib/queue/workers/code-gen-worker.ts` -- 代码生成 Worker
...
**验证：**
  ...
  - 失败重试机制工作
**产出：**
  - `src/lib/queue/workers/code-gen-worker.ts`
**预估耗时：** 4小时
**可并行：** 否
...
**前置条件：** STEP-048完成
**执行：**
  1. 创建 `src/lib/engines/codegen/execution-orchestrator.ts` -- 执行编排器
...
**验证：**
  ...
  - 进度面板实时更新
**产出：**
  - `src/lib/engines/codegen/execution-orchestrator.ts`
...
**前置条件：** STEP-005完成
**执行：**
  1. 创建 `src/lib/translation/name-mapper.ts` -- 技术名称到业务名称映射
  ...
  2. 创建 `src/lib/translation/mapping-table.ts` -- 静态映射表（覆盖 80% 场景）
  3. 映射表支持项目级自定义扩展
**验证：**
  - 10 个常见文件路径翻译正确
  - 10 个技术状态翻译为 PM 可读语言
...
**前置条件：** STEP-051完成
**执行：**
  1. 创建 `src/lib/translation/status-translator.ts` -- 状态翻译器
     - Pipeline Phase -> PM 可见消息（例如："正在为您的项目编写代码..."）
     - 节点状态变化 -> 进度流条目
     - 错误场景的诚实翻译（不隐瞒失败，但用 PM 语言解释）
  2. 创建 `src/lib/translation/progress-formatter.ts` -- 进度流格式化
     - 将技术事件格式化为进度面板条目
     - 图标选择、颜色编码、时间戳
...
**执行：**
  1. 创建 `src/lib/translation/ai-translator.ts` -- AI 辅助翻译（覆盖映射表未命中的 20%）
...
**验证：**
  ...
  - 翻译层出错时正确降级
**产出：**
  - `src/lib/translation/ai-translator.ts`
...
**前置条件：** STEP-048完成
**执行：**
  1. 创建 `src/lib/queue/workers/quality-gate-worker.ts` -- 质量关卡 Worker
  2. 实现 ESLint 检查步骤：
     - 从 R2 下载节点生成的代码文件
     - 运行 ESLint 检查
...
**验证：**
  ...
  - 无错误的代码 -> 通过
**产出：**
  - `src/lib/queue/workers/quality-gate-worker.ts`（ESLint 部分）
...
**前置条件：** STEP-057完成
**执行：**
  1. 创建 `src/lib/quality/auto-fixer.ts` -- AI 自动修复器
     - 输入：错误信息 + 原始代码
     - 调用 AI 生成修复后的代码
     - 替换原文件，重新运行检查
     - 最多 3 次修复尝试
...
**前置条件：** STEP-059完成
**执行：**
  1. 创建 `src/lib/deploy/build-service.ts` -- 项目构建服务
     - 从 R2 下载所有代码文件 -> 组装完整 Next.js 项目
     - 生成 package.json（动态依赖列表）
     - 执行 `npm install` + `npm run build`
     - 构建产物打包
  2. 创建 `src/lib/queue/workers/deploy-worker.ts` -- 部署 Worker
     - 处理构建 + 部署的完整流程
...
**验证：**
  ...
  - 构建失败 -> AI 修复 -> 重新构建成功
**产出：**
  - `src/lib/deploy/build-service.ts`
  - `src/lib/queue/workers/deploy-worker.ts`
**预估耗时：** 4小时
**可并行：** 否
...
**执行：**
  ...
  2. 创建 `src/lib/deploy/cloudflare-deployer.ts` -- Cloudflare Pages 部署
...
**执行：**
  ...
  2. 创建 `src/lib/screenshot/screenshot-service.ts` -- 截图服务
     - 浏览器池管理（最少 2 实例、最大 10 实例）
     - capture(url, options) -> 截图并上传到 R2
     - 支持 viewport 配置（默认 1280x720）
     - 支持全页面截图和元素截图
     - 等待策略：networkidle（页面完全加载后截图）
  3. 创建 `src/lib/queue/workers/screenshot-worker.ts` -- 截图 Worker
...
**验证：**
  ...
  - 截图文件正确存储在 R2
**产出：**
  - `src/lib/screenshot/screenshot-service.ts`
  - `src/lib/queue/workers/screenshot-worker.ts`
**预估耗时：** 3小时
**可并行：** 否
...
**前置条件：** STEP-062完成
**执行：**
  1. 创建 `src/lib/screenshot/smoke-test.ts` -- 冒烟测试
     - 验证首页可访问（HTTP 200）
     - 验证所有页面级路由可达
     - 检查是否有控制台错误
     - 检查是否有 404 资源
...
**验证：**
  ...
  - 故意破坏一个路由 -> 冒烟测试捕获失败
**产出：**
  - `src/lib/screenshot/smoke-test.ts`
**预估耗时：** 2小时
**可并行：** 否
...
**前置条件：** STEP-064完成
**执行：**
  1. 创建 `src/lib/deploy/build-fixer.ts` -- 构建失败修复器
...
**前置条件：** STEP-064完成
**执行：**
  1. 创建 `src/lib/pipeline/checkpoint-manager.ts` -- 检查点管理器
...
**产出：**
  - `src/lib/pipeline/checkpoint-manager.ts`
...
**前置条件：** STEP-064完成
**执行：**
  1. 创建 `src/lib/deploy/preview-manager.ts` -- 预览 URL 管理
     - 生成、查询、失效预览 URL
     - 预览环境自动清理：30 天不活跃则删除
  2. 创建 `src/lib/deploy/project-exporter.ts` -- 项目导出
...
**前置条件：** STEP-060完成
**执行：**
  1. 创建 `src/lib/deploy/dependency-manager.ts` -- 依赖管理
...
**验证：**
  ...
  - 安装后构建成功
**产出：**
  - `src/lib/deploy/dependency-manager.ts`
**预估耗时：** 2.5小时
...
**前置条件：** STEP-062完成
**执行：**
  1. 创建 `src/lib/acceptance/question-generator.ts` -- 验收问题生成器
     - 输入：功能节点信息 + 截图 URL
     - 为每个功能生成 2-3 个是/否问题
...
**验证：**
  ...
  - 问题不重复
**产出：**
  - `src/lib/acceptance/question-generator.ts`
...
**前置条件：** STEP-071完成
**执行：**
  1. 创建 `src/lib/acceptance/issue-classifier.ts` -- 问题分类器
     - PM 说"否"时，提供 4 类问题选项：
       ...
       4. 不符合预期（整体效果与想象不同）
     - 每类问题下有细分选项
  2. 创建 `src/lib/acceptance/feedback-interpreter.ts` -- PM 反馈解释器
...
**前置条件：** STEP-072完成
**执行：**
  1. 创建 `src/lib/acceptance/fix-engine.ts` -- 修复引擎
     - 输入：结构化修复需求 + 原始代码 + 截图
     - AI 分析问题 -> 生成修复后的代码
     - 修复代码替换原文件 -> 重新通过质量关卡
...
**验证：**
  ...
  - 升级处理机制工作
**产出：**
  - `src/lib/acceptance/fix-engine.ts`
...
**执行：**
  ...
  6. SQL 注入防护：使用 Supabase Client 参数化查询
